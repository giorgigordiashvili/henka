...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function loadLanguageData(language) {
    try {
      ...
      showToast(`${language.toUpperCase()} language data loaded successfully`, "success");
    } catch (error) {
  ...
  async function saveContent() {
    ...
    try {
      ...
      const response = await fetch("/.netlify/functions/github-api/update-content", {
        method: "POST",
        headers: {
  ...
  async function handleImageUpload(e) {
    ...
    const imageName = document.getElementById("image-name").value || imageFile.name.split(".")[0];
  ...
  async function loadAssets(folderPath = "public/assets") {
    try {
      foldersContainer.innerHTML = '<div class="loading">Loading folders...</div>';
      imagesGrid.innerHTML = '<div class="loading">Loading images...</div>';

      const response = await fetch(`/.netlify/functions/github-api/list-assets`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      if (data.images && data.images.length > 0) {
        ...
        data.images.forEach((image) => {
          ...
          imageItem.querySelector(".copy-path").addEventListener("click", (e) => {
